...
void setPixels(){
  if(digitalRead(SOLS[0]) == 1){
    for(int i=0; i<4; i++) { // For each pixel... 
      ...
      pixels.setPixelColor(pixelSetA[i], pixels.Color(random(255), random(255), random(255)));
    ...
    if(digitalRead(SOLS[1]) == 1){
      for(int i=0; i<4; i++) { // For each pixel... 
        pixels.setBrightness(10);    
        pixels.setPixelColor(pixelSetB[i], pixels.Color(random(255), random(255), random(255)));
        pixels.show();   // Send the updated pixel colors to the hardware.
      }
    }
